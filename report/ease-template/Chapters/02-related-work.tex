...
to ours---treated package classification as a zero-shot and retrieval-augmented
task, evaluating open and proprietary models on a dataset of malicious and
benign PyPI packages~\cite{ease2025rag}. That study augmented generation with
knowledge bases built from YARA rules, GitHub security advisories, and
malicious \texttt{setup.py} examples, and reported both that LLM-only
detection leaves room for improvement and that retrieved context helps.
...
``Many Hands Make Light Work'')~\cite{lamps2025}. Our work differs in scope
